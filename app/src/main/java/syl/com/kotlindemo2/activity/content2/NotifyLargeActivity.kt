 ...
 * 发送大图通知
 */
class NotifyLargeActivity : BaseActivity() {
    ...
    companion object {
        private val TAG = "NotifyLargeActivity"
    }
...
//        val titleBean = intent.getParcelableExtra("title") as TitleBean
        ...
        tv_spinner.setOnClickListener {
            selector("请选择大视图风格", styles) { dialog, i ->
...
//        sendLargeNotify(title, message, style)
        sendNotification(1, title, message, style!!)
        toast("大视图消息已推送到通知栏。")
    }
     ...
     * 发送通知
     */
    fun sendNotification(notifyId: Int, title: String, content: String, style: NotificationCompat.Style) {
        val builder = getNotification(title, content, style)
    ...
    private fun getNotification(
        ...
        //声明一个“前往”按钮的动作意图
        val confirmIntent = intentFor<MainActivity>()
        val piConfirm = PendingIntent.getActivity(
            this,
            R.string.app_name, confirmIntent, PendingIntent.FLAG_UPDATE_CURRENT
        )

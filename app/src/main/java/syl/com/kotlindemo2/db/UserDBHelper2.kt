package syl.com.kotlindemo2.db
 ...
 * @Called
 */
class UserDBHelper2 (var context: Context, private var DB_VERSION: Int=CURRENT_VERSION) : ManagedSQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    companion object {
        private val TAG = "UserDBHelper2"
        var DB_NAME = "user2.db" //数据库名称
        var TABLE_NAME = "user_info" //表名称
        var CURRENT_VERSION = 1 //当前的最新版本，如有表结构变更，该版本号要加一
        private var instance: UserDBHelper2? = null
        @Synchronized
        fun getInstance(ctx: Context, version: Int=0): UserDBHelper2 {
            if (instance == null) {
                //如果调用时没传版本号，就使用默认的最新版本号
                instance = if (version>0) UserDBHelper2(ctx.applicationContext, version)
                else UserDBHelper2(ctx.applicationContext)
            }
            return instance!!
        }
    }
    ...
    fun insert(info: UserInfo2): Long {
        val infoArray = mutableListOf(info)
        return insert(infoArray)
    }

    fun insert(infoArray: MutableList<UserInfo2>): Long {
        var result: Long = -1
        for (i in infoArray.indices) {
            val info = infoArray[i]
            var tempArray: List<UserInfo2>
            // 如果存在同名记录，则更新记录
            // 注意条件语句的等号后面要用单引号括起来
    ...
    @JvmOverloads
    fun update(info: UserInfo2, condition: String = "rowid=${info.rowid}"): Int {
        val cv = ContentValues()
        cv.put("name", info.name)
        cv.put("age", info.age)
        cv.put("height", info.height)
    ...
    fun queryByPhone(phone: String): UserInfo2 {
        val infoArray = query("phone='$phone'")
        val info: UserInfo2 = if (infoArray.size>0) infoArray[0] else UserInfo2()
        return info
    }

    fun deleteAll(): Int = delete("1=1")

    fun queryAll(): List<UserInfo2> = query("1=1")
